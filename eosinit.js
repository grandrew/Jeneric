        ...
        this.stomp.onmessageframe = function(frame) {
            ...
            */
            ...
            var reviver = function (key, value) {
                return blob_parse(key, value, blobCount, rq);
            };
...
function getWrappedBlob(bid, blobCount, blobObject, req) {
    ...
    dr.onfail = function (status, txt) {
        if(window.console) console.log("wrappedBlob get failure for request "+req.id+" with status "+status);
    };
    dr.addArg(_POST, "blob_session", hubConnection.___SESSIONKEY);
    dr.addArg(_POST, "blobid", bid);
    dr.getURL( "/base64get" );
...
function blob_parse (key, value, blobCount, rq) {
    var d;
    if (typeof value == 'string' &&
        ...
        if (d[0] == key) {
            ...
            if(window.google && google.gears) getBlob(value, blobCount, blobObject, rq); // TODO
            else getWrappedBlob(value, blobCount, blobObject, rq); // TODO
