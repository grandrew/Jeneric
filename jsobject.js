...
function eos_hubRequest(vm, cs, sUri, sMethod, lArgs, timeout) {
    ...
    if(typeof(timeout) == "Number") {
        ...
        setTimeout(cs, timeout); // document that timeout is milliseconds
    }
...
function eos_rcvEvent(data) {
    ...
    if(rq.result) {
        ...
        if(!(rq.id in __eos_requests)) return; // silently fail?
        ...
        var dest = t.getChild(lURI); // XXX check for what will HUB do with URI here
        ...
        if(typeof(dest) == "string") {
            ...
            var cbo = function (rs) {
                var dr = new DataRequestor();
                dr.addArg(_POST, "request", JSON.stringify(rs)); // document to never use __defineProperty__
                dr.getURL("/urihub");
                // XXX no return value check...???
                // TODO: the result may be lost!! and the waiter will wait forever then
                //       make the system retry on network error several times...
            };
