...
function Jnaric() {
    ...
    this.global.Lock.prototype.release = function () {
        if(this.lock > 0) {
            // set exception
            __tihs.cur_stack.EXCEPTION = THROW;
            __tihs.cur_stack.my.x2.result = (new __tihs.global.Error("release unlocked lock"));
        }
        //++this.lock; // is like someone may have requested a lock again, dont release it!
        var desc = this.___wait_queue.shift();
        
        if(!desc) {
            ++this.lock; // just leave unlocked
            return;
        }
